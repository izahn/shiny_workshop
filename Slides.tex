...
\item
  ...
  \newline \texttt{install.packages(c("tidyverse",\ "plotly",\ "leaflet",\ "DT"))}
\end{itemize}
\end{frame}
...
\item
  Nursing homes:
  \url{https://www.hbs.edu/covid-19-business-impact/Topics/Health-Care-Management-and-Policy/COVID-19-Skilled-Care-Center-Scorecard}
\end{itemize}
...
\FunctionTok{names}\NormalTok{(storms)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{help.search}\NormalTok{(}\StringTok{"Output"}\NormalTok{, }\AttributeTok{package =} \StringTok{"shiny"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{help.search}\NormalTok{(}\StringTok{"Input"}\NormalTok{, }\AttributeTok{package =} \StringTok{"shiny"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  console to determine the input corresponding to the row number of the
  clicked observation.
\item
